...
public final class ManifestTest extends AndroidTestCase
{
    ...
    /**
     * Verifies that the plug-in exports a single
     * {@link com.twofortyfouram.locale.Intent#ACTION_QUERY_CONDITION} BroadcastReceiver.
     */
    @SmallTest
     ...
     * {@link com.twofortyfouram.locale.Intent#ACTION_EDIT_CONDITION}.
     * 
    ...
    private static List<ResolveInfo> getPluginActivities(final Context context)
    {
        ...
        for (final ResolveInfo x : context.getPackageManager()
                                          .queryIntentActivities(new Intent(
                                                                            com.twofortyfouram.locale.Intent.ACTION_EDIT_CONDITION), PackageManager.GET_ACTIVITIES))
        {
    ...
    private static List<ResolveInfo> getPluginReceivers(final Context context)
    {
        ...
        for (final ResolveInfo x : context.getPackageManager()
                                          .queryBroadcastReceivers(new Intent(
                                                                              com.twofortyfouram.locale.Intent.ACTION_QUERY_CONDITION), PackageManager.GET_INTENT_FILTERS))
        {
            if (packageName.equals(x.activityInfo.packageName))
            {
                result.add(x);
