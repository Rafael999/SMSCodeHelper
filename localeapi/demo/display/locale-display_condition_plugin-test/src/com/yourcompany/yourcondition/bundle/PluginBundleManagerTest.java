...
public final class PluginBundleManagerTest extends AndroidTestCase
{
    ...
    public static void testExtraConstants()
    {
         ...
         */
        assertEquals("com.yourcompany.yourcondition.extra.BOOLEAN_STATE", PluginBundleManager.BUNDLE_EXTRA_BOOLEAN_STATE); //$NON-NLS-1$
    ...
    {
        final Bundle bundle = PluginBundleManager.generateBundle(getContext(), true);
        assertNotNull(bundle);

        assertEquals(2, bundle.keySet().size());

        assertTrue(bundle.getBoolean(PluginBundleManager.BUNDLE_EXTRA_BOOLEAN_STATE));
        assertEquals(Constants.getVersionCode(getContext()), bundle.getInt(PluginBundleManager.BUNDLE_EXTRA_INT_VERSION_CODE));
    }

    /**
     * Tests {@link PluginBundleManager#generateBundle(android.content.Context, boolean)} with the display
     * off.
     */
    @SmallTest
    public void testGenerateBundle_off()
    {
        final Bundle bundle = PluginBundleManager.generateBundle(getContext(), false);
        assertNotNull(bundle);

        assertEquals(2, bundle.keySet().size());

        assertFalse(bundle.getBoolean(PluginBundleManager.BUNDLE_EXTRA_BOOLEAN_STATE));
        assertEquals(Constants.getVersionCode(getContext()), bundle.getInt(PluginBundleManager.BUNDLE_EXTRA_INT_VERSION_CODE));
    }
