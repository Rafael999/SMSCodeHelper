...
package com.yourcompany.yourcondition.bundle;
...
public final class BundleScrubberTest extends AndroidTestCase
{
    ...
    /*
     * Note: From within this unit test, it isn't possible to verify the BundleScrubber's behavior for an
     * extra coming from another process. The problem is that any private serializable put into the Bundle
     * from within the same process is available to the app's class loader since the app owns the class. These
     * unit tests simply verify the behavior of the BundleScrubber with valid inputs.
     */
    ...
    /**
     * Verifies that passing a valid Intent to {@link BundleScrubber#scrub(Intent)} returns false and does not
     * mutate the Intent.
     */
    @SmallTest
    ...
    /**
     * Verifies that passing a valid Bundle to {@link BundleScrubber#scrub(Bundle)} returns false and does not
     * mutate the Bundle.
     */
    @SmallTest
