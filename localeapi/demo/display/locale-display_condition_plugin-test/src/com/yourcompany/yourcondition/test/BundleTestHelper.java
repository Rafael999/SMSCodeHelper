...
package com.yourcompany.yourcondition.test;
 ...
 * <p>
 * This class is also a subclass of {@link AndroidTestCase} so that it can verify its own implementation
 * details.
 */
public class BundleTestHelper extends AndroidTestCase
{
    ...
    /**
     * A helper method that attempts to serialize all of the objects inside the Bundle. This helps test that a
     * Bundle only contains objects that can be serialized and doesn't contain Parcelable objects.
     */
    private static void checkBundle(final Bundle bundle)
    {
        ...
        try
        {
            ...
            for (final String key : bundle.keySet())
            {
                ...
                else if (value instanceof Serializable) // allows objects that implement both parcelable and
                                                        // serializable
                {
                    try
                    {
                    ...
                    {
                        throw new RuntimeException(
                                                   String.format("Object associated with key %s is not available to the Android ClassLoader", key)); //$NON-NLS-1$
                    }
                    ...
                    {
                        throw new RuntimeException(
                                                   String.format("Object associated with key %s couldn't be serialized", key)); //$NON-NLS-1$
                    }
                ...
                {
                    throw new RuntimeException(
                                               String.format("Key \"%s\"'s value %s isn't Serializable.  Only primitives or objects implementing Serializable can be stored.  Parcelable is not stable for long-term storage.", key, bundle.get(key))); //$NON-NLS-1$
                }
            }
        }
        finally
